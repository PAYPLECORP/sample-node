...
router.post('/auth', (req, res, next) => {
    ...
    const params = {
        cst_id: process.env.CST_ID || '',                  // 가맹점 ID
        custKey: process.env.CUST_KEY || '',               // 가맹점 Key
...
router.post('/refund', (req, res) => {
    ...
    post('http://localhost:3000/auth', {PCD_PAYCANCEL_FLAG: "Y"}).then(r => {
...
router.post('/taxsaveReg', (req, res) => {
    //(기존 가맹점인증 라우터 이용 - POST http://localhost:3000/auth)
    //[현금영수증 취소 요청]가맹점인증 요청변수: PCD_PAY_WORK = 'TSREG'(발행요청) | 'TSCANCEL'(발행취소)
    post('http://localhost:3000/auth', {PCD_PAY_WORK: 'TSREG'}).then(r => {
...
router.post('/taxsaveCan', (req, res) => {
    //(기존 가맹점인증 라우터 이용 - POST http://localhost:3000/auth)
    //[현금영수증 취소 요청]가맹점인증 요청변수: PCD_PAY_WORK = 'TSREG':발행요청 || 'TSCANCEL':발행취소
    post('http://localhost:3000/auth', {PCD_PAY_WORK: 'TSCANCEL'}).then(r => {
...
router.post('/paycheck', (req, res) => {
    ...
    post('http://localhost:3000/auth', {PCD_PAYCHECK_FLAG: 'Y'}).then(r => {
